...
\textit{Agreement} dari kedua anotator tersebut adalah 0.924. Berdasarkan hasil tersebut, dapat dilihat bahwa Giza++ secara umum dapat menghasilkan \textit{alignment} cukup baik untuk korpus identik yang digunakan. Namun demikian, kesalahan ataupun kelemahan yang sering terjadi dari \textit{alignment} Giza adalah pada kasus-kasus seperti:
...
Pada hasil akurasi (F-Score) tabel tersebut, dapat terlihat bahwa pada fitur F1, tujuh dari sepuluh (70\%) buah kata memiliki nilai performa diatas baseline, enam dari sepuluh kata (60\%) diatas baseline pada fitur F2, tujuh dari sepuluh kata (70\%) diatas baseline pada fitur F3, dan delapan dari sepuluh kata (80\%) diatas baseline pada fitur F4. Rerata performa dari hasil tersebut dapat dilihat pada tabel \ref{table:performa-rerata-wsd}.
...
Berdasarkan rata-rata dari nilai F-Score tersebut, dapat dilihat bahwa untuk kesepuluh data \textit{sampling} yang digunakan, rerata dari F-Score semua performa mengungguli sistem dengan algoritma baseline.
...
Berdasarkan sepuluh grafik diatas, dapat kita lihat bahwa sembilan dari sepuluh \textit{sample} kata yang dicoba, hanya satu kata yang tidak pernah memiliki performa diatas baseline yaitu kata "halaman". Sembilan kata lainnya selalu memiliki performa di atas baseline untuk minimal satu dari keempat fitur yang dicoba. Pada delapan buah \textit{sample}, fitur F2 memiliki akurasi di bawah F1 walaupun sudah menggunakan \textit{word embedding}. Pada empat buah kata, F4 yang merupakan fitur gabungan F1 dan F3 memiliki performa yang lebih baik dari fitur F1. Penggunaan berbagai skenario fitur tersebut menunjukan bahwa fitur F1 dan F4 memiliki performa minimal sama atau di atas baseline pada sembilan dari sepuluh \textit{sample} (90\%). Dengan jumlah persentase 60\% fitur F2 mengungguli performa dari baseline, F2 (\textit{word embedding}) memiliki persentase lebih kecil dari fitur lainnya yang memiliki persentase lebih tinggi jika dibandingkan dengan baseline. Performa yang lebih rendah dari fitur lain ini bisa disebabkan karena domain yang berbeda antara \textit{training} pada saat membentuk model \textit{word embedding} (perbedaan pada gaya bahasa pada Wikipedia dan korpus identik). Namun demikian, persentase terbesar dari \textit{sampling} sepuluh kata di atas diperoleh pada fitur F4 yang memiliki performa lebih baik dari baseline pada delapan kasus dari sepuluh kata tersebut (80\%).